...
export default function ProgressBar({ progressVal }) {
  ...
  const [displayOuterAnimatedText, setDisplayOuterAnimatedText] =
    useState("block");
  ...
  return (
    <View style={styles.progressBarContainer}>
      ...
      >
        <Text style={[styles.progressText, { display: displayAnimatedText }]}>
          {isNaN(progressVal) ? 0 : progressVal}%
        </Text>
      </Animated.View>
      <Text
        style={[
          styles.progressText,
          { display: displayOuterAnimatedText, left: "10px" },
        ]}
      >
        {isNaN(progressVal) ? 0 : progressVal}%
      </Text>
